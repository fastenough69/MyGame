...
namespace Objects {
	...
	protected:
		...
		float speed{ 200.0f };
		Obj() = default;
		Obj(std::shared_ptr<Render::ProgramShader> pr, Sprite::SpriteSize& sz, glm::vec2& pos, glm::vec2& vel, float speed = 200.0f) :
			shProgram{ pr }, size{ sz }, position{ pos }, velocity{ vel }, speed{ speed }
			{	}
	public:
		virtual ~Obj() { table_state.clear(); };
		virtual void set_pos(glm::vec2 ps) = 0;
		virtual  void add_state(const std::string name, std::shared_ptr<Render::Texture2D>, std::shared_ptr<Sprite::SpriteAnim>) = 0;
	...
	public:
		...
		virtual void add_state(const std::string name, std::shared_ptr<Render::Texture2D>, std::shared_ptr<Sprite::SpriteAnim>) override;
		...
		virtual glm::vec2 get_position() const override { return position; };
